          ...
          <div key={zip.id} className="rounded-xl border border-blue-100 p-4">
            ...
            {zip.status === "RESERVED" && (
              <div className="mt-3 flex gap-2">
                <Link href={`/dashboard/realtor/checkout/${zip.id}`} className="primary-btn text-xs">Checkout</Link>
                <Link href={`/dashboard/realtor/onboarding/${zip.id}`} className="secondary-btn text-xs">Onboarding</Link>
              </div>
            )}
          </div>
        ))}
